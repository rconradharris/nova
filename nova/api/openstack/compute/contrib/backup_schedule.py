...
class FakeTempoClient(object):
    def __init__(self, url):
        self.fake_tasks = []
        self.task_create("daily_backup",
                         "dae7b332-de5d-4fdd-a82b-1354e5b69f88",
                         "0 2 *")

        self.task_create("weekly_backup",
                         "dae7b332-de5d-4fdd-a82b-1354e5b69f88",
                         "0 2 6")
    ...
    def task_delete(self, uuid):
        ...
        self.fake_tasks.remove(to_delete)

    def task_create(self, task, instance_uuid, recurrence):
        self.fake_tasks.append(dict(instance_uuid=instance_uuid,
                                    task=task,
...
class BackupScheduleController(object):
    def __init__(self, fake=False):
        if fake:
            self.tempo_client = FakeTempoClient(FLAGS.tempo_url)
        else:
            if tempo_available:
